...
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", ["http://localhost:5500", "http://localhost:5501", "http://127.0.0.1:5500", "http://127.0.0.1:5501"])
...
def _to_face_static_url(image_path: str) -> str:
    rel_path = os.path.relpath(image_path, KNOWN_FACES_DIR)
    url_path = "/".join(quote(part) for part in rel_path.split(os.sep))
    return f"/face-images/{url_path}"
...
def get_face_image_url(face_label: Optional[str]) -> Optional[str]:
    ...
    for folder_path in candidate_dirs:
        ...
        if image_path:
            return _to_face_static_url(image_path)
